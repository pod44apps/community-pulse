...
export default function EditAppModal({ app, onClose, onUpdate }) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showImageGenerator, setShowImageGenerator] = useState(false);
  const [formData, setFormData] = useState({
    name: app.name || "",
    description: app.description || "",
    category: app.category || "organization",
    icon: app.icon || "",
    iconColor: app.iconColor || "text-blue-600",
    iconBg: app.iconBg || "bg-blue-100"
  });
  ...
  // Find the color option that matches the current iconColor
  const getCurrentColorValue = () => {
    const found = colorOptions.find(c => c.textClass === formData.iconColor);
    return found ? found.value : "blue";
  };
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update your community app details
          </DialogDescription>
        </DialogHeader>
